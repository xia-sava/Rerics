...
impl StatusBarView {
    ...
    pub fn new(parent: &(impl GuiParent + 'static), position: (i32, i32), size: (i32, i32)) -> Self {
        ...
        let inner = Rc::new(Inner { left: RefCell::new(String::new()), right: RefCell::new(String::new()) });
    ...
    fn paint_to(&self, dc: &w::HDC, fonts: &FontSet, cw: i32, ch: i32) -> w::AnyResult<()> {
        ...
        if !left.is_empty() {
            ...
            fonts.draw_text(dc, &left, rect, flags)?;
            left_edge = pad + fonts.width(dc, &left);
        }
        ...
        if !right.is_empty() {
            ...
            fonts.draw_text(dc, &right, rect, flags)?;
        }
        Ok(())
    }
}
