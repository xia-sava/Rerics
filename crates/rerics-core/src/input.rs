...
impl Command {
    ...
    const ALL: &'static [(Command, &'static str, &'static str, &'static str)] = {
        ...
        &[
            ...
            (ContextMenu, "contextMenu", "コンテキストメニュー", "選択（無ければカーソル）した項目のシェルのコンテキストメニューを開く"),
            ...
            (CompareDialog, "compareDialog", "同名ファイル選択", "両ペインの同名ファイルを比較し、選んだ条件に合う項目を選択する"),
            (DirectoryCompare, "directoryCompare", "ディレクトリ比較", "アクティブと反対ペインのディレクトリを比較し、差分を結果一覧に表示する"),
            (DirectoryCompareDialog, "directoryCompareDialog", "ディレクトリ比較（条件指定）", "日付・サイズの比較条件と抽出範囲を選んでディレクトリ比較を実行する"),
            (FindFile, "findFile", "ファイル検索", "現在地以下を再帰検索し、名前マスクに一致するファイルを結果一覧に表示する"),
            (FindFileDialog, "findFileDialog", "ファイル検索（条件指定）", "ファイル名・日付・サイズの条件を指定して現在地以下を再帰検索する"),
            ...
            (About, "about", "バージョン情報", "バージョンとライセンス情報のダイアログを開く"),
            (CheckUpdate, "checkUpdate", "自動更新", "新しいビルドが公開されていないか確認し、あればダウンロードして更新する"),
    ...
    pub fn meta(self) -> CommandMeta {
        ...
        match self {
            ...
            FindFile => CommandMeta {
                summary,
                args: &[ArgSpec {
                    name: "mask",
                    ty: ArgType::Str,
                    required: false,
                    doc: "ファイル名マスク（カンマ/空白区切り・`!` で除外・省略時は何もしない）",
                }],
                examples: &[r#"findFile("*.txt")"#, r#"findFile("*.rs, *.toml")"#],
            },
