	...
	fn default_message_handlers(&self) {
		...
		self.0.raw_base.base().on().wm_close(move || {
			if let Ok(hparent) = self2.0.raw_base.base().hwnd().GetWindow(co::GW::OWNER) {
				...
				// Read the saved focus handle BEFORE destroying the modal.
				// DestroyWindow dispatches WM_NCDESTROY synchronously, which clears
				// this window's event closures -- including the very closure running
				// here and its captured `self2`. Touching `self2` after DestroyWindow
				// would be a use-after-free. The handle has no Drop, so a bitwise read
				// out of the cell is a plain copy.
				let hchild_prev_focus_parent =
					unsafe { self2.0.hchild_prev_focus_parent.get().read() };
